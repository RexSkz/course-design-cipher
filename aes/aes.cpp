...
void GF28_add(uint8_t a[], uint8_t b[], uint8_t d[]) {
	d[0] = a[0]^b[0];
	d[1] = a[1]^b[1];
	d[2] = a[2]^b[2];
	d[3] = a[3]^b[3];
...
int AESEncrypt(uint8_t *src, uint8_t *dst, int length, int *rlength, uint8_t *key, int keylength) {
    int nop=length%16, i;
    nop = (16 - nop)%16;
    int slength = length + nop;
    uint8_t *source = (uint8_t *)malloc(slength), *w;
    memcpy(source, src, length);
    for(i = length; i<slength; ++i) {
        source[i]=(uint8_t)nop;
    }
    switch (keylength) {
        default:
    ...
    KeyExpansion(key, w);
    uint8_t *p = source;
    for(i=0; i<slength; i+=16) {
        self_encrypt(source+i, dst+i, w);
    }
    *rlength = slength;
    free(source);
    free(w);
...
int AESDecrypt(uint8_t *src, uint8_t *dst, int length, int *rlength, uint8_t *key, int keylength) {
    int nop=length%16, i;
    if(nop)
        return -1;
    uint8_t *w;
